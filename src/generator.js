...
// CRITICAL: Category-based prioritization with geographic scoring and diversity cap
  prioritizeByCategory(articles) {
    ...
    const prioritized = articles.map(article => {
      ...
      let geoBonus = 0;
      if (australianScore > 0) geoBonus = Math.min(australianScore * 8, 24);
      if (foreignScore > 0 && australianScore === 0) geoBonus = -15;
  ...
  categorizeArticle(article, segment = null) {
    ...
    const enforcementIndicators = [
      ...
      'crackdown', 'blitz', 'operation', 'compliance check', 'roadside check',
...
- Be direct and specific. Name the action, who does it, and when.
- GOOD: "Pull your last 90 days of driver work diaries and verify sign-off against scheduled runs. Do this before your next NHVR audit window."
- GOOD: "Brief your operations team on this change at this week's toolbox meeting. Update your SMS risk register to reflect the new obligation."
- GOOD: "Check your PBS vehicle approval conditions against the new SRT parameters. If outside tolerance, notify your accreditation manager today."
- BAD: "You might consider reviewing your compliance processes when convenient."
- BAD: "It could be worthwhile to think about whether this affects your operations."
...
- Stop short of predicting prosecution, fines, or legal outcomes for specific situations — the disclaimer covers the analysis; the tip covers the action.
...
ACTION TIP REQUIREMENTS — CRITICAL:
- Be direct. Tell the driver what to do, not what they might want to think about doing.
- GOOD: "Check your work diary before you start today's run. If you are within 2 hours of your limit, tell your dispatcher now — not after you have rolled."
- GOOD: "Do a full pre-trip brake check on every trailer coupling point this week. NHVR is targeting load restraint and brake compliance."
- GOOD: "If your truck is in the affected model range, book it in for inspection before your next long haul. Do not wait for a defect notice."
- BAD: "You might want to consider checking your work diary when you get a chance."
- BAD: "It could be worth having a think about whether this affects your driving."
- Keep it practical: things a driver can do on the job, before a shift, or when talking to their depot.
- Do NOT predict fines, licence loss, or legal outcomes for specific situations — the disclaimer covers that.
